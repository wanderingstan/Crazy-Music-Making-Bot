...
async def music_generation(prompt, model_version="melody", duration=8, continuation=False,
                           normalization_strategy="loudness", top_k=250, top_p=0, temperature=1,
                           classifier_free_guidance=3, output_format="wav", seed=None):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(music_url) as response:
            if response.status == 200:
                music_filename = f'temp_music.{output_format}'
